...
-- Dumping data for table hochipohub.categories: ~4 rows (approximately)
...
-- Dumping data for table hochipohub.users: ~3 rows (approximately)
...
-- Dumping structure for table hochipohub.vendor_applications
CREATE TABLE IF NOT EXISTS `vendor_applications` (
  ...
  KEY `fk_vendor_applications_user` (`user_id`),
  KEY `fk_vendor_applications_reviewer` (`reviewed_by`),
  CONSTRAINT `fk_vendor_applications_reviewer` FOREIGN KEY (`reviewed_by`) REFERENCES `users` (`user_id`) ON DELETE SET NULL,
  CONSTRAINT `fk_vendor_applications_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
