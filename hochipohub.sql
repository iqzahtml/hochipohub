-- ============================================================
-- hochipohub
-- FRESH DATABASE FOR PIEx
-- Based on the supplied database structure
-- MySQL 8
-- ============================================================
...
-- ============================================================
-- WARNING:
-- THIS REMOVES THE EXISTING hochipohub DATABASE
...
CREATE TABLE `users` (
    ...
    `role` ENUM(
        ...
        'vendor',
        'admin'
    ) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'customer',
    `status` ENUM(
        'active',
        'inactive',
        'pending',
...
CREATE TABLE `vendors` (
    ...
    `delivery_method` ENUM(
        ...
        'Postage',
        'Both'
    ) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Both',

    `postage_fee` DECIMAL(10,2) NOT NULL DEFAULT '0.00',
    `allow_vendor_delivery` TINYINT(1) NOT NULL DEFAULT '0',
    `cod_enabled` TINYINT(1) NOT NULL DEFAULT '0',
    `vendor_delivery_fee` DECIMAL(10,2) NOT NULL DEFAULT '0.00',
    ...
    KEY `idx_vendors_location` (`latitude`, `longitude`),
...
CREATE TABLE `vendor_orders` (
    ...
    KEY `idx_vendor_orders_status` (`vendor_status`),
...
CREATE TABLE `payments` (
    ...
    `gateway_order_reference` VARCHAR(150) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
        ON UPDATE CURRENT_TIMESTAMP,
    ...
    KEY `idx_payment_transaction` (`transaction_reference`),
...
CREATE TABLE `commission` (
    ...
    KEY `fk_commission_vendor_order` (`vendor_order_id`),
    KEY `idx_commission_vendor` (`vendor_id`),
...
CREATE TABLE `reviews` (
    ...
    `status` ENUM(
        'Visible',
        'Hidden'
    ) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'Visible',
    ...
    KEY `idx_reviews_order_detail` (`order_detail_id`),
...
CREATE TABLE `conversations` (
    ...
    KEY `idx_conversations_vendor` (`vendor_id`),
...
CREATE TABLE `contact_messages` (
    ...
    KEY `idx_contact_messages_replied_by` (`replied_by`),
...
CREATE TABLE `vendor_applications` (
    ...
    KEY `fk_vendor_applications_reviewer` (`reviewed_by`),
    KEY `idx_vendor_application_user` (`user_id`),
    KEY `idx_vendor_application_status` (`status`),
